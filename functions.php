...
function checkUserByEmail1($email){
...
function checkUserByUsername1($username){
    ...
    global $conn;
     ...
     $stmt = $conn->prepare(
            'SELECT * FROM profile WHERE mactricno=?'
     );
...
function checkUserByUsername1a($username){
    
    global $conn;
    
   
     $stmt = $conn->prepare(
            'SELECT * FROM profile WHERE mactricno=? AND status=1'
     );
...
function checkUserActivation($username){
    ...
    //Store them into database
     $stmt = $conn->prepare(
            'SELECT * FROM profile WHERE email=? and status=1'
     );
...
function checkUserActivations($username){
...
function checkUserActivation2($username){
    ...
    //Store them into database
     $stmt = $conn->prepare(
            'SELECT * FROM studentdetails WHERE RegNo=?'
     );
...
function verifyUserAccount($code){
    ...
    global $conn;
     ...
     $stmt = $conn->prepare(
            "UPDATE profile SET status = 1 , reset_code = '' WHERE reset_code = ?"
     );
...
function verifyUserAccount1($code){
...
function checkUserByCode($code){
    ...
    //Store them into database
     $stmt = $conn->prepare(
            'SELECT * FROM profile WHERE reset_code = ?'
     );
...
function checkUserByCode1($code){
