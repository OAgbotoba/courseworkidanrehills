...
function checkUserByUsername($username){
    ...
    global $conn;
     ...
     $stmt = $conn->prepare(
            'SELECT * FROM biodata WHERE username=?'
     );
...
function checkUserByUsername1($username){
    ...
    global $conn;
     ...
     $stmt = $conn->prepare(
            'SELECT * FROM staffinfo WHERE username=?'
     );
...
function checkUserActivation($username){
    ...
    //Store them into database
     $stmt = $conn->prepare(
            'SELECT * FROM biodata WHERE username=? AND is_active=1'
     );
...
function checkUserActivation1($username){
    ...
    //Store them into database
     $stmt = $conn->prepare(
            'SELECT * FROM staffinfo WHERE username=? AND status=1'
     );
